...
type AdminCreateParams = { name?: string; price?: string; accessibility?: boolean; amenities?: string[] };
...
export const test = base.extend<{ adminCreateRoom: (opts?: AdminCreateParams) => Promise<void> }>({
  adminCreateRoom: async ({ page }, use) => {
    ...
    await use(async (opts: AdminCreateParams = {}) => {
      const { name = '104', price = '20', accessibility = true, amenities = ['WiFi','Refreshments','TV','Safe','Radio','Views'] } = opts;
      await loginHelper.quickLogin(process.env.ADMIN_USERNAME!, process.env.ADMIN_PASSWORD!, '/admin');
      ...
      for (const a of amenities) {
        if (a === 'Refreshments' || a === 'Radio') {
          await adminRooms.page.locator('div').filter({ hasText: new RegExp(`^${a}$`) }).nth(1).click();
        } else {
